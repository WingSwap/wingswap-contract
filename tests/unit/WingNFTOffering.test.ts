...
import { MockWBNB, WingNFT, SimpleToken, SimpleToken__factory, WNativeRelayer } from "../../typechain";
...
describe("WingNFTOffering", () => {
  ...
  let wingNFT: ModifiableContract;
  let ogOffering: WingNFTOffering;
  ...
  // binding
  let ogOfferingAsAlice: WingNFTOffering;
  let ogOfferingAsABob: WingNFTOffering;
  ...
  beforeEach(async () => {
    ({ stakingTokens, signatureFn, wbnb, wNativeRelayer, wingNFT, ogOffering, startingBlock, priceModel } =
      await waffle.loadFixture(ogOfferingUnitTestFixture));
    [deployer, alice, bob, dev] = await ethers.getSigners();

    // binding
    ogOfferingAsAlice = WingNFTOffering__factory.connect(ogOffering.address, alice);
    ogOfferingAsABob = WingNFTOffering__factory.connect(ogOffering.address, bob);
  ...
  describe("#readyToSellNFT()", () => {
    context("if the seller is not a governance", () => {
      it("should revert", async () => {
        ...
        ).to.revertedWith("WingNFTOffering::onlyGovernance::only GOVERNANCE role");
      });
    });
    ...
    context("when the quoteToken is address(0)", () => {
      it("should revert", async () => {
        ...
        ).to.revertedWith("WingNFTOffering::_setQuoteBep20::invalid quote token");
      });
    });
    ...
    context("when invalid block", () => {
      ...
      context("when starting >= end block", () => {
        it("should revert", async () => {
          ...
          ).to.revertedWith("WingNFTOffering::_setWingNFTMetadata::invalid start or end block");
        });
      });
    });
    ...
    context("when duplicate sell", () => {
      it("should revert", async () => {
        await expect(
          ...
          .to.emit(ogOffering, "SetWingNFTMetadata")
          .withArgs(0, 1, startingBlock.add(100), startingBlock.add(101));
        const metadata = await ogOffering.wingNFTMetadata(0);
        ...
        await expect(
          ogOffering.readyToSellNFT(0, 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)
        ).to.revertedWith("WingNFTOffering::_readyToSellNFTTo::duplicated entry");
      });
    });

    it("should create a correct wingNFTMetadata", async () => {
      await expect(
        ogOffering.readyToSellNFT(0, 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)
      )
        .to.emit(ogOffering, "SetQuoteBep20")
  ...
  describe("#buyNFT", () => {
    ...
    context("when reaching a maximum cap", () => {
      it("should revert", async () => {
        ...
        await expect(ogOfferingAsAlice.buyNFT(0)).to.revertedWith(
          "WingNFTOffering::_decreaseCap::maximum mint cap reached"
        );
      });
    });
    ...
    context("when a token is a native", () => {
      context("with msg.value", () => {
        context("when amount != msg.value", () => {
          it("should revert", async () => {
            ...
            ).to.revertedWith("WingNFTOffering::_safeWrap:: value != msg.value");
          });
        });
      });
      context("when the user reach a buy limit", () => {
        it("should revert", async () => {
          ...
          let ogMetadata = await ogOffering.wingNFTMetadata(0);
          ...
          ).to.revertedWith("WingNFTOffering::_buyNFTTo::exceed buy limit");
          ...
          expect(await wingNFT.ownerOf(0)).to.eq(await alice.getAddress());
          expect(await wingNFT.ownerOf(1)).to.eq(await alice.getAddress());
          expect(await wingNFT.ownerOf(2)).to.eq(await alice.getAddress());
          ogMetadata = await ogOffering.wingNFTMetadata(0);
    ...
    context("when a token is not a native", () => {
      context("with msg.value", () => {
        it("should revert", async () => {
          ...
          ).to.revertedWith("WingNFTOffering::_safeWrap:: baseToken is not wNative");
        });
      });
      ...
      context("when the user reach a buy limit", () => {
        it("should revert", async () => {
          ...
          await expect(ogBuyLimit.counter).to.eq(2);
          // shouldn't buy because limit count is 2
          await expect(ogOfferingAsAlice.buyNFT(0)).to.revertedWith("WingNFTOffering::_buyNFTTo::exceed buy limit");
  ...
  describe("#setWingNFTMetadata", () => {
    it("should set a correct param", async () => {
      await ogOffering.setWingNFTMetadata([
        {
          nftCategoryId: 0,
          cap: 1,
      ...
      const category0Metadata = await ogOffering.wingNFTMetadata(0);
      const category1Metadata = await ogOffering.wingNFTMetadata(1);
    ...
    context("when setting a zero address as a price model argument", () => {
      it("should revert", async () => {
        await expect(ogOffering.setPriceModel(constants.AddressZero)).to.revertedWith(
          "WingNFTOffering::permit::price model cannot be address(0)"
        );
      });
    });
    ...
    context("when setting a zero address as a price model argument", () => {
      it("should revert", async () => {
        await expect(ogOffering.setQuoteBep20(0, constants.AddressZero)).to.revertedWith(
          "WingNFTOffering::_setQuoteBep20::invalid quote token"
        );
      });
    });

...
contract Stake is ERC20('Bean Token', 'BEAN'), IStake, Ownable {
  ...
  function transferFrom(address _from, address _to, uint256 _amount) public virtual override returns (bool) {
    ...
    _approve(_from, _msgSender(), allowance(_from, _msgSender()).sub(_amount, "Stake::transferFrom::transfer amount exceeds allowance"));
    _moveDelegates(_delegates[_from], _delegates[_to], _amount);
    return true;
  }

  /// @notice Mint `_amount` BEAN to `_to`. Must only be called by MasterChef.
  ...
  /// @notice Burn `_amount` BEAN to `_from`. Must only be called by MasterChef.
